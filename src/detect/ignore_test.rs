...
#[test]
fn bare_ipv4_accepted_as_host() {
    let cidrs = vec!["192.168.1.50".to_string()];
    let list = IgnoreList::new(&cidrs, false).unwrap();
    assert!(list.is_ignored(&IpAddr::V4(Ipv4Addr::new(192, 168, 1, 50))));
    assert!(!list.is_ignored(&IpAddr::V4(Ipv4Addr::new(192, 168, 1, 51))));
}

#[test]
fn bare_ipv6_accepted_as_host() {
    let cidrs = vec!["2001:db8::5".to_string()];
    let list = IgnoreList::new(&cidrs, false).unwrap();
    let ip: IpAddr = "2001:db8::5".parse().unwrap();
    assert!(list.is_ignored(&ip));
    let other: IpAddr = "2001:db8::6".parse().unwrap();
    assert!(!list.is_ignored(&other));
}
