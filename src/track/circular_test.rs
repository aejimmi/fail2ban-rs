...
fn threshold_with_find_time_zero() {
    ...
    buf.push(100);
    buf.push(100);
    // All same timestamp: new - old = 0, find_time = 0. The window is
    // inclusive (0 <= 0), so an instantaneous burst still trips the threshold.
...
fn threshold_span_exactly_find_time_counts() {
    ...
    buf.push(1000);
    buf.push(1300);
    buf.push(1600); // span = 600
    assert!(buf.is_full());
    assert!(buf.threshold_reached(600)); // 600 <= 600
    assert!(!buf.threshold_reached(599)); // 600 > 599
}
