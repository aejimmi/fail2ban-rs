...
impl FirewallBackend for NftablesBackend {
    async fn init(&self, jail: &str, ports: &[String], protocol: &str) -> Result<()> {
        ...
        self.run_nft(&["add", "table", "inet", "fail2ban-rs"]).await?;
        // Create base chain for filtering input.
        self.run_nft(&[
            "add", "chain", "inet", "fail2ban-rs", "f2b-chain",
            "{ type filter hook input priority -1; policy accept; }",
        ]).await.ok(); // ignore if already exists
        // Create IPv4 set.
        let set_name = format!("f2b-{jail}");
        self.run_nft(&[
            "add", "set", "inet", "fail2ban-rs", &set_name,
            "{ type ipv4_addr; flags interval; }",
        ]).await?;
        // Create IPv6 set.
        let set_v6 = format!("f2b-{jail}-v6");
        self.run_nft(&[
            "add", "set", "inet", "fail2ban-rs", &set_v6,
        ...
        if !ports.is_empty() {
            ...
            let rule_v4 = format!(
                "{protocol} dport {{ {port_list} }} ip saddr @{set_name} reject"
            );
            self.run_nft(&[
                "add", "rule", "inet", "fail2ban-rs", "f2b-chain", &rule_v4,
            ]).await?;
            let rule_v6 = format!(
                "{protocol} dport {{ {port_list} }} ip6 saddr @{set_v6} reject"
            );
            self.run_nft(&[
                "add", "rule", "inet", "fail2ban-rs", "f2b-chain", &rule_v6,
            ]).await?;
        } else {
    ...
    async fn teardown(&self, jail: &str) -> Result<()> {
        ...
        self.run_nft(&["flush", "set", "inet", "fail2ban-rs", &set_name]).await.ok();
        self.run_nft(&["delete", "set", "inet", "fail2ban-rs", &set_name]).await.ok();
        self.run_nft(&["flush", "set", "inet", "fail2ban-rs", &set_v6]).await.ok();
        self.run_nft(&["delete", "set", "inet", "fail2ban-rs", &set_v6]).await.ok();
        Ok(())
    }
